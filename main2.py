...
class Question0(webapp2.RequestHandler):
	...
	template_values = {
            'id': "0",
			'someQuestion' : "Привет! \nДобро пожаловать на небольшой интерактивный квест, в конце которого ты узнаешь 9 интересных фактов о Генеральном партнере форума - компании «Норникель»!\nЧитай внимательно!\nПолученная информация станет ключевым фактором твоей победы в викторине от компании, которая пройдёт в 17:30 на сцене после мастер-класса!\nНачнем с простенького: \n2+2*2=?"}
...
class Question7(webapp2.RequestHandler):
	...
	template_values = {
            'id': "7",
			'someQuestion' : "Осталось совсем чуть-чуть до финала! <br>Вот наш вопрос:<br>На заводе работают три смены, которые сменяют друг друга. Каждая смена работает по 8 часов. Первая смена выходит на работу в 10 утра, во сколько заканчивается рабочий день у 3 смены?"}
        path = os.path.join(os.path.dirname(__file__), 'foo.html')
        self.response.out.write(template.render(path, template_values))
...
class Question8(webapp2.RequestHandler):
	...
	template_values = {
            'id': "8",
			'someQuestion' : "Поздравляю, ты дошёл до финала! Решай задачку и получай последний факт о Генеральном партнере форума!<br><br>Именно столько миллиардов долларов составила выручка компании «Норникель» в первом полугодии 2017 года. Чтобы узнать ответ запиши через запятую (без пробелов) период и группу, в которых находится Аргон."}
        path = os.path.join(os.path.dirname(__file__), 'foo.html')
        self.response.out.write(template.render(path, template_values))
